...
namespace GABBE {
	...
	void Init() {
		...
		if (res[2] != std::to_string(version * 2) && (res.size() < 3 || res[2] == "0" || res[2] == std::to_string(Safety::GetSelfSize()))) {
			MessageBoxA(NULL, res[2].c_str(), "Error!", MB_OK | MB_ICONEXCLAMATION);
		...
		if (res.size() >= 4) {
			...
			if (atoi(res[2].c_str()) > version) {
				...
				if (link.length() >= 4) {
					if (MessageBoxA(NULL, "An update is available!\nPress ok to download it.", "auto updater", MB_OKCANCEL) == IDOK) {
						...
						updateStarted = true;
						MessageBoxA(NULL, "Please restart application to apply update!", "auto updater", MB_OK);
						exit(0);
					}
					else MessageBoxA(NULL, "Sorry, you have to install latest version in order to launch GABB!", "auto updater", MB_OK), exit(0);
				}
